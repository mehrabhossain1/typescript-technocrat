...
const addArrow = (num1: string, num2: string): string => num1 + num2;
...
const arr: string[] = ["1", "4", "5", "6"];
// const newArray = arr.map((elem: number): number => elem * elem);
const newStringArr = arr.map((elem: string): string => elem + elem);

const arr2: number[] = [1, 2, 3];
const newArr: number[] = arr2.map((elem: number): number => elem * elem);
